...
int main(int argc, char *argv[])
{
    ...
    cout << "Modified Mono-Inertial for LaMAR" << endl;
                ...
                vector<string> &vstrImages, vector<double> &vTimeStamps)
{
    ...
    while(!fTimes.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            double t;
            ss >> t;
            vTimeStamps.push_back(t/1e6); // Changed from 1e9 to 1e6 for LaMAR
...
void LoadIMU(const string &strImuPath, vector<double> &vTimeStamps, vector<cv::Point3f> &vAcc, vector<cv::Point3f> &vGyro)
{
    ...
    while(!fImu.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            vTimeStamps.push_back(data[0]/1e6); // Changed from 1e9 to 1e6 for LaMAR
